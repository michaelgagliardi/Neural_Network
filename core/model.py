...
class Custom_Model:
    ...
    def finalize(self):
        ...
        if isinstance(self.layers[-1], activation_softmax) and isinstance(self.loss, loss_categorical_cross_entropy):
            self.softmax_classifer_output = activation_softmax_loss()
    ...
    def train(self, X, y, *, epochs=1, batch_size = None, print_every=1, validation_data = None):
        ...
        for epoch in range(1, epochs + 1):
            ...
            print(f'training, ' + 
                  ...
                  f'loss: {epoch_loss} (' + 
                  f'data_loss: {epoch_data_loss}, ' + 
                  f'reg_loss: {epoch_regularization_loss}), ' + 
    ...
    def backward(self, output, y):
        if self.softmax_classifer_output is not None:
            self.softmax_classifer_output.backward(output, y)
            self.layers[-1].dinputs = self.softmax_classifer_output.dinputs
    ...
    def predict(self, X, *, batch_size = None):
        ...
        for step in range(prediction_steps):
            if batch_size is None:
                ...
                batch_X = X[step*batch_size:(step+1)*batch_size]
            batch_ouput = self.forward(batch_X, training=False)
